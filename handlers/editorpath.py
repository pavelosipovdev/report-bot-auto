...
async def main_menu_bt_constructor_editor(message: types.Message, state: FSMContext):
    ...
    buy_data = utils.connectors.db_sql_buy_editor_select(message.text.upper())
...
async def main_menu_bt_constructor_editor(message: types.Message, state: FSMContext):
    await message.answer(text="Пожалуйста подождите")
    comission_data = utils.connectors.db_sql_comission_editor_select(message.text.upper())
